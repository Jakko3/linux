...
static inline int rt5033_init_fast_charge(struct rt5033_charger *charger)
{
	...
	ret = regmap_update_bits(charger->rt5033->regmap, RT5033_REG_CHG_CTRL3,
			RT5033_CHGCTRL3_TIMER_EN_MASK, RT5033_INT_TIMER_ENABLE);
	if (ret) {
		dev_err(charger->dev, "Failed regmap update\n");
		return -EINVAL;
	}

	/* Set fast-charge timeout to 4 hours */
	ret = regmap_update_bits(charger->rt5033->regmap, RT5033_REG_CHG_CTRL3,
			RT5033_CHGCTRL3_TIMER_MASK, RT5033_FAST_CHARGE_TIMER4);
	if (ret) {
		dev_err(charger->dev, "Failed regmap update\n");
		return -EINVAL;
	}
...
static int rt5033_charger_reg_init(struct rt5033_charger *charger)
{
	...
	/* Set minimum input voltage regulation (MIVR) */
	ret = regmap_update_bits(charger->rt5033->regmap, RT5033_REG_CHG_CTRL4,
						...
						struct platform_device *pdev)
{
	 ...
	 * regulated voltage. In this driver, external sensing register value
	 * is 10 mili ohm.
	 */
